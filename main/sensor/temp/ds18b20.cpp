...
{
    uint8_t scratch[9];
    gflags.validTemperature = false; // fixme
    _converting = false;
    ...
    // fixme connect to old code
    float val = raw / 16.0f;
    static float temperature = 0., temp_prev = 0.;
    if ( temperature == 0. ) {
        temperature = val;
    }
    temperature += (val - temperature) * 0.3; // A bit low pass as strategy against toggling
    if (abs(temperature - temp_prev) > 0.1) {
        float tr = std::round(temperature * 10) / 10;
        OAT.set(tr);
        ESP_LOGI(FNAME, "NEW temperature=%2.1f", tr);
